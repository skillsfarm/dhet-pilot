...
from dhet_admin.components import ComponentRegistry
from dhet_admin.enums import ActionVariant
from dhet_admin.sections import BaseSection
from dhet_admin.widgets import (
    DhetAdminAdminMoneyWidget,
    DhetAdminAdminSelect2Widget,
    DhetAdminAdminSplitDateTimeWidget,
)
...
def action_list(context: RequestContext) -> str:
    ...
    return render_to_string(
        "dhet_admin/helpers/tab_actions.html",
...
def tab_list(context: RequestContext, page: str, opts: Options | None = None) -> str:
    ...
    return render_to_string(
        "dhet_admin/helpers/tab_list.html",
        request=context["request"],
        context=data,
    )
...
@register.inclusion_tag(
    "dhet_admin/templatetags/preserve_changelist_filters.html",
    takes_context=True,
    ...
    for fieldset in adminform:
        for line in fieldset:
            for field in line:
                ...
                if isinstance(
                    field.field.field.widget, DhetAdminAdminSplitDateTimeWidget
                ) or isinstance(field.field.field.widget, DhetAdminAdminMoneyWidget):
...
def changeform_condition(
    ...
    elif isinstance(field.field.field.widget, DhetAdminAdminSelect2Widget):
...
@register.simple_tag(name="dhet_admin_querystring", takes_context=True)
def dhet_admin_querystring(context, *args, **kwargs):
    """
...
def header_title(context: RequestContext) -> str:
    ...
    return render_to_string(
        "dhet_admin/helpers/header_title.html",
        request=context.request,
        context={
            "parts": parts,
        },
