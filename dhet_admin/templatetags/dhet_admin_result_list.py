...
from dhet_admin.views import DatasetChangeList
from dhet_admin.widgets import DhetAdminBooleanWidget
        ...
        if link_in_col(first, field_name, cl):
            ...
            if (
                ...
                result_repr = mark_safe(
                    str(bf)
                    + render_to_string(
                        "dhet_admin/helpers/form_errors.html", {"errors": bf.errors}
                    )
                )
...
class DhetAdminResultList(ResultList):
    def __init__(
        self,
        instance: Model,
        form: ModelForm | None,
...
def results(cl: ChangeList):
    if hasattr(cl, "formset") and cl.formset:
        for res, form in zip(cl.result_list, cl.formset.forms):
            yield DhetAdminResultList(res, form, items_for_result(cl, res, form))
    else:
        for res in cl.result_list:
            yield DhetAdminResultList(res, None, items_for_result(cl, res, None))
...
@register.tag(name="dhet_admin_result_list")
...
def paginator_number(cl: ChangeList, i: str | int) -> str | SafeText:
    ...
    if i == cl.page_num:
        return render_to_string(
            "dhet_admin/helpers/pagination_current_item.html",
            {
                "number": i,
            },
        )
...
@register.tag(name="dhet_admin_search_form")
def dhet_admin_search_form_tag(parser, token):
    return InclusionAdminNode(
        parser,
        token,
        func=dhet_admin_search_form,
        template_name="search_form.html",
        takes_context=False,
    )


@register.tag(name="dhet_admin_admin_actions")
def dhet_admin_admin_actions_tag(parser, token):
    return InclusionAdminNode(
        parser,
        token,
        func=admin_actions,
