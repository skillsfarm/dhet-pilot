...
class DhetAdminModelAdminChecks(ModelAdminChecks):
    def check(self, admin_obj: BaseModelAdmin, **kwargs) -> list[checks.Error]:
        return [
            *super().check(admin_obj, **kwargs),
            *self._check_dhet_admin_action_permission_methods(admin_obj),
        ]

    def _check_dhet_admin_action_permission_methods(
        self, obj: Any
    ) -> list[checks.Error]:
        """
