...
from dhet_admin.dataclasses import DhetAdminAction
...
class ActionModelAdminMixin:
    ...
    def get_dhet_admin_action(self, action: str) -> DhetAdminAction:
        """
        Converts action name into DhetAdminAction object.
        """
        method = self._get_instance_method(action)

        return DhetAdminAction(
    ...
    def get_actions_list(self, request: HttpRequest) -> list[DhetAdminAction]:
        """
        Filters `actions_list` by permissions and returns list of DhetAdminAction objects.
        """
        return self._filter_dhet_admin_actions_by_permissions(
            request, self._get_base_actions_list()
        )

    def get_actions_detail(
        self, request: HttpRequest, object_id: int
    ) -> list[DhetAdminAction]:
        """
        Filters `actions_detail` by permissions and returns list of DhetAdminAction objects.
        """
        return self._filter_dhet_admin_actions_by_permissions(
            request, self._get_base_actions_detail(), object_id
        )

    def get_actions_row(self, request: HttpRequest) -> list[DhetAdminAction]:
        """
        Filters `actions_row` by permissions and returns list of DhetAdminAction objects.
        """
        return self._filter_dhet_admin_actions_by_permissions(
    ...
    def get_actions_submit_line(
        ...
        """
        Filters `actions_submit_line` by permissions and returns list of DhetAdminAction objects.
        """
        return self._filter_dhet_admin_actions_by_permissions(
    ...
    def _get_base_actions_list(self) -> list[DhetAdminAction]:
        """
        Returns list of DhetAdminAction objects for `actions_list`.
        """
        return [
            self.get_dhet_admin_action(action)
            for action in self._extract_action_names(self.actions_list)
        ]

    def _get_base_actions_detail(self) -> list[DhetAdminAction]:
        """
        Returns list of DhetAdminAction objects for `actions_detail`.
        """
        return [
            self.get_dhet_admin_action(action)
    ...
    def _get_base_actions_row(self) -> list[DhetAdminAction]:
        """
        Returns list of DhetAdminAction objects for `actions_row`.
        """
        return [
            self.get_dhet_admin_action(action)
    ...
    def _get_base_actions_submit_line(self) -> list[DhetAdminAction]:
        """
        Returns list of DhetAdminAction objects for `actions_submit_line`.
        """
        return [
            self.get_dhet_admin_action(action)
    ...
    def _get_instance_method(self, method_name: str) -> Callable:
        ...
        If it does not exist or is not callable, it raises DhetAdminException
        """
        try:
            method = getattr(self, method_name)
        except AttributeError as e:
            raise DhetAdminException(
                f"Method {method_name} specified does not exist on current object"
            ) from e

        if not callable(method):
            raise DhetAdminException(f"{method_name} is not callable")
    ...
    def _get_actions_navigation(
        ...
        allowed_actions: list[DhetAdminAction],
        object_id: str | None = None,
    ) -> list[str | dict]:
        """
        ...
        def get_action_by_name(name: str) -> DhetAdminAction:
            """
        ...
        def get_action_path(action: DhetAdminAction) -> str:
            """
        ...
        def get_action_attrs(action: DhetAdminAction) -> dict:
            """
    ...
    def _filter_dhet_admin_actions_by_permissions(
        self,
        request: HttpRequest,
        actions: list[DhetAdminAction],
