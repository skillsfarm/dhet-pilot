...
from dhet_admin.dataclasses import DropdownItem, Favicon, SearchResult
...
from dhet_admin.utils import convert_color


class DhetAdminAdminSite(AdminSite):
    default_site = "dhet_admin.admin.DhetAdminAdminSite"
    settings_name = "DHET_ADMIN"

    def __init__(self, name: str = "admin") -> None:
        from dhet_admin.forms import AuthenticationForm
    ...
    def search(
        ...
        template_name = "dhet_admin/helpers/search_results.html"
        ...
        cache_key = f"dhet_admin_search_{hashlib.sha256(force_bytes(search_key_base)).hexdigest()}"
        cache_results = cache.get(cache_key)

        if extended_search:
            template_name = "dhet_admin/helpers/command_results.html"
    ...
    def password_change(
        ...
        from dhet_admin.forms import AdminOwnPasswordChangeForm
